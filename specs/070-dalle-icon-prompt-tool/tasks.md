...
- [X] T001 Confirm `pip install openai pillow` succeeds in this environment; confirm `OPENAI_API_KEY`
      can be read from the environment (`python -c "import os; print(bool(os.environ.get('OPENAI_API_KEY')))"`).
- [X] T002 Add `docs/weathericons/generated/` to `.gitignore` (scratch output, never committed —
...
- [X] T003 [P] [US1] Create `docs/weathericons/icon_prompt_data.py` with `TYPE_CATEGORY` (12
...
- [X] T004 [US1] In `icon_prompt_data.py`, add `BASE_STYLE` and `GUARDRAILS` (the shared opening
...
- [X] T005 [US1] In `icon_prompt_data.py`, add `CLOTHING_BY_BAND["kvinna"]` for all six bands
      ...
      line.
- [X] T006 [US1] In `icon_prompt_data.py`, add `SCENE_BY_TYPE_BAND_TIME` for the six dry types
...
- [X] T007 [US1] Add a module-level validation check at the bottom of `icon_prompt_data.py` that
...
- [X] T008 [P] [US1] Add `WIND_FRAGMENT` to `icon_prompt_data.py` — the new windswept-effect prompt
      sentence (research.md §5), not present in any legacy file.

### Tool implementation for User Story 1

- [X] T009 [US1] Create `docs/weathericons/generate_icon.py` with `argparse` CLI accepting
...
- [X] T010 [US1] In `generate_icon.py`, add a `build_prompt(request) -> str` function that
...
- [X] T011 [US1] In `generate_icon.py`, add a `generate_image(prompt) -> bytes` function calling
...
- [X] T012 [US1] In `generate_icon.py`, add a `check_transparency(image_bytes) -> tuple[bool, int]`
...
- [X] T013 [US1] In `generate_icon.py`, add a `verify_content(image_bytes, request) -> tuple[bool, str]`
...
- [X] T014 [US1] In `generate_icon.py`, wire the pipeline together: build prompt → generate →
...
- [X] T015 [P] [US1] Create `docs/weathericons/test_icon_prompt_data.py` (plain `unittest`):
...
- [X] T016 [P] [US1] Create `docs/weathericons/test_generate_icon.py` (plain `unittest`, no network
...
- [X] T021 Update `docs/weathericons/generated/`'s presence in `.gitignore` is committed (T002),
